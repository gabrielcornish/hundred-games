...
namespace AC
{
	...
	public class RuntimeInventory : MonoBehaviour
	{
		...
		public void HighlightItem (int _id, HighlightType _type)
		{
			...
			KickStarter.eventManager.Call_OnInventoryHighlight (highlightItem, _type);
		}
				...
				foreach (InvItem item in KickStarter.inventoryManager.items)
				{
					if (item.carryOnStart)
					{
						...
						if (KickStarter.settingsManager.playerSwitching == PlayerSwitching.Allow && 
							//item.carryOnStartNotDefault &&
							!KickStarter.settingsManager.shareInventory)
						{
							if (!item.carryOnStartNotDefault)
							{
								playerIDs.Add (KickStarter.settingsManager.GetDefaultPlayerID ());
							}
							else
							{
								foreach (int carryOnStartID in item.carryOnStartIDs)
								{
									playerIDs.Add (carryOnStartID);
								}
							}
						}
						else
						...
						foreach (int playerID in playerIDs)
						{
							if (item.canCarryMultiple && item.useSeparateSlots)
							{
								for (int i = 0; i < item.count; i++)
								{
									...
									if (playerID != -1)
									{
										AddToOtherPlayer (item.id, item.count, playerID, false);
									}
									else
									{
